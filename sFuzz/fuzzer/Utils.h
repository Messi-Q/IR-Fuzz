...
ContractInfo parseJson(string jsonFile, string contractName, bool isMain)
{
    ...
    {
        stringstream output;
        output << "[x] File " + jsonFile + " is empty, failed in compiling!" << endl;
        cout << output.str();
        exit(0);
    }
    file.seekg(ios::beg);
    ...
    for (auto key : root.get_child("contracts"))
    {
        ...
        if (keyname.substr(keyname.find(":") + 1, keyname.size()) == contractName)
        {
            fullContractName = keyname;
            break;
        }
    }
    ...
    pt::ptree::path_type srcmapRuntimePath(
        "contracts|" + fullContractName + "|srcmap-runtime", '|');
    ...
    for (auto it : root.get_child("sources"))
    {
        ...
        while (stack.size() > 0)
        {
            ...
            {
                if (item.get<string>("attributes.stateMutability") == "view" ||
                    item.get<string>("attributes.stateMutability") == "pure" ||
                    item.get<string>("attributes.stateMutability") == "constant")
                {
...
string toContractName(directory_entry file)
{
    ...
    string fileNameWithoutExtension =
        fileName.find(".") != string::npos ? fileName.substr(0, fileName.find(".")) : fileName;
    string contractName =
        fileNameWithoutExtension.find("_0x") != string::npos ?
...
void forEachFile(string folder, string extension, function<void(directory_entry)> cb)
{
    ...
    for (auto& file : boost::make_iterator_range(directory_iterator(folderPath), {}))
    {
        if (is_directory(file.status()))
            forEachFile(file.path().string(), extension, cb);
        if (!is_directory(file.status()) && boost::ends_with(file.path().string(), extension))
            cb(file);
    }
}
...
string compileSolFiles(string folder, string attackerName)
{
    stringstream ret;
    string filePath = folder + "/" + attackerName + ".sol";
    ret << "solc";
    ret << " --combined-json abi,bin,bin-runtime,srcmap,srcmap-runtime,ast " + filePath;
    ret << " > " + filePath + ".json";
    ret << endl;
...
string fuzzJsonFiles(string contracts, string assets, int duration, int case_num, int mode,
    int reporter, string attackerName, bool is_prefuzz)
{
    stringstream ret;
    ...
    forEachFile(contracts, ".sol", [&](directory_entry file) {
        ...
        {
            ret << " --testcases " + to_string(case_num);
            ret << endl;
        }
    });
    return ret.str();
